...
typedef struct EffectiveFreqData {
	...
	double *dCurrentFreq;	///< Supported for programs from Matisse
	double *dCurrentTemp;
}EffectiveFreqData;
...
typedef struct CPUParameters
{
	...
	float fTDCLimit_CCD;			///< TDC CCD Current Limit(A)
	float fTDCValue_CCD;			///< TDC CCD Current value(A)
	float fEDCLimit_CCD;			///< EDC CCD Current Limit(A)
	float fEDCValue_CCD;			///< EDC CCD Current value(A)
}CPUParameters;
